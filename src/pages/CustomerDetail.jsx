  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-5">
      ...
      <div className="mt-6 space-y-4">
        <div>
          <p className="text-xs text-gray-400 mb-2 font-semibold uppercase tracking-wider">Customer Info</p>
      ...
      {/* Activity */}
      <div className="mt-6">
        <h4 className="text-xs text-gray-400 mb-2 font-semibold uppercase tracking-wider">Activity</h4>
        <div className="bg-gray-50/50 rounded-lg border p-3 space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Registration:</span>
            <span className="font-medium text-gray-800">{formatDate(customer.join_date)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Last Purchase:</span>
            <span className="font-medium text-gray-800">{formatDate(customer.last_purchase)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Source:</span>
            <span className="font-medium text-gray-800 capitalize">{customer.user_source || "-"}</span>
          </div>
        </div>
      </div>
      ...
      {/* Order Summary */}
      <div className="mt-6">
        <h4 className="text-xs text-gray-400 mb-2 font-semibold uppercase tracking-wider">Order Overview</h4>

        <div className="grid grid-cols-3 gap-2">
          <div className="border rounded-lg p-3 text-center bg-gray-50/50">
            <p className="text-xl font-bold text-gray-800">{customer.order_count ?? 0}</p>
            <p className="text-[10px] uppercase font-semibold text-gray-500 mt-1">Total</p>
          </div>

          <div className="border rounded-lg p-3 text-center bg-green-50/30">
            <p className="text-xl font-bold text-green-600">
              {customer.completed_orders ?? 0}
            </p>
            <p className="text-[10px] uppercase font-semibold text-gray-500 mt-1">Completed</p>
          </div>

          <div className="border rounded-lg p-3 text-center bg-red-50/30">
            <p className="text-xl font-bold text-red-500">
              {customer.cancelled_orders ?? 0}
            </p>
            <p className="text-[10px] uppercase font-semibold text-gray-500 mt-1">Cancelled</p>
          </div>
        </div>
      </div>
    </div>
